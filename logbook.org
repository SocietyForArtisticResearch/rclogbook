...
*** The audio player does not work in https://www.researchcatalogue.net/view/1043548/1043684 VERY IMPORTANT to NOT CHANGE THE PUBLICATION DATE
*** Menu of expositions in profile and of tools in workspace does not work in Windows 10, version 19041
*** The account 1116029 Wang Yi-Ru (wangyiru668@163.com) cannot be deleted
** Reported Bugs
...
We would like to get rid of the Font warning messages, by making sure only fonts are used that RC can host itself :-)
Currently, there are many fonts (Verdana, Terminal), that are not available everywhere, due to licensing.

We have selected a list of public domain fonts, which the RC can self-host:
...
Also, in the expositions settings it should be possible to upload your own font (.woff), give it a name and then use it in the HTML / Text tools.

For new expositions, these should replace all the fonts.
For all other expositions, authors should still be able to use the old fonts (they should see both the old and new fonts).
