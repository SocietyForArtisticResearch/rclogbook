...
*** Graphical editor: undo button and better versioning of exposition content
*** Merging of text-based and graphical editors
...
*** TODO Fix multilingual abstracts
   1. The main language as selected by the author should also define which language of the abstract is shown as the primary on the META page.
   2. Authors should be able to edit/add the translations of abstracts.
   3. The main language of an exposition should be displayed in the metadata page.
   4. The abstract displayed in the Navigation bar abstract box should be the same as the language of the exposition, and there should be a dropdown to switch between languages (if translations are available).
...
** Released features
*** DONE searchbox is missing from front-page in responsive mode
*** DONE fix default license in text editor (or maybe postpone?)
*** DONE Better error reporting 
*** DONE Admins can view snapshots that were made on submission.
*** DONE Implement fix for font-boosting both in text-based and graphical expositions
...
*** Export as pdf does not work in the text editor
*** Should videos be exported as well when export as epub in the text editor? Now they are not exported

** Reported Bugs
*** https://www.researchcatalogue.net/view/433112/616170 footnote problem (reported by Tero)
