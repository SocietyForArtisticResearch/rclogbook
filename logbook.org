...
*** exposition metadata export for journalsg
...
*** Fullscreen does not work in embedded videos. example 
...
*** position of tools can be made negative by mouse, but not edited by number in the style tab. (reporter: JAR, closed but not live)
*** Fullscreen and Downloads on HTML-import videos (NTNU)
*** Footnotes -> dissapearing scrollbar problem when opening certain notes (SAR-1262)
*** Restoring an exposition while it was "in review", the "in review" status is inherited (SAR-1261)
** Fixed Bugs
...
** DONE open keywords set https://dev.researchcatalogue.net/api/keywords/open 
  note: body should be something like {"value":"I am a keyword really"}
** DONE open keywords get
