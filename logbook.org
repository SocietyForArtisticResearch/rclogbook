...
*** searchable inbox
*** adding time/date filter in advanced search.
...
*** state share status clearly (no gray color)
*** state display abstract on/off setting
*** state show keywords on the object in the box
*** state easier group management: tagging for profile organization / change group button for item
*** state collapse all groups 
*** state cleanup of CV details edit profile
*** state customizable display of external publications (easy way)
*** state see list of collab/co-autherships of user (ex. academia.org)
*** state see own followers
*** state follow institutions
*** state portal profile pages 
...
** increase audio quality of zencoder in videos (currently, uses pretty low bitrate, filters out everything above 15k)
...
** fix multilingual abstracts
   1. The main language of an exposition should be displayed in the metadata page.
   2. The abstract displayed in the Navigation bar abstract box should be the same as the language of the exposition, and there should be a dropdown to switch between languages (if translations are available).
** make downloading big exports more fail safe: big expositions cause timeouts, like https://www.researchcatalogue.net/view/568007/568008 
** when user uploads a video with a problematic resolution, show user hint what would work.
** add grouping tags for users within portal user list (for organising)
** publication tags for exposition, to separate it from keywords
** merge keywords of expositions and media repo
** make the media repo github repository public (clean up old state first)
** duplicate page command within editors (instead of copy paste tools)
** work migration to media repo set
** fix "add all" button in collaboration message dialog
** collaboration notifications removed from messages, should be prompt or notification. State should be live from the current state
** picture tool should use tool default instead of intrensic size 
** duplicate accounts (automatic detection)
** collaboration, finding the right person if you only have an email.
** add "published by" to articles on the frontpage, published by JAR for example.
** send email to NTNU admin when FEIDE student has logged in first time, creating a new account
** media url's in text-based editor, not always translated to live url (https://www.re catalogue.net/text-editor/simple-media-resource?research=547070&simple-media=945677)
** snapshot stored when admin changes revision state from "in revision" -> "in review"
** add trashcan button for comments on pages that can be seen by the commenter.
...
** front page - cycle through columns doesn't work (other columns update as well)
** front page, abstracts cropped to elipses.
...
** Clicking tool should not open defaults 
